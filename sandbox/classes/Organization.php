...
class Organization
{
    ...
    public function UPDATE_ORG($org, $org_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'>Update successful <a href='http://localhost/scpc/sandbox/add_org.php'><span class='glyphicon glyphicon-backward'></span> </a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_ORG($org_id) {
        ...
        if($stmt->execute()) {
            header("location: http://localhost/scpc/sandbox/add_org.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
