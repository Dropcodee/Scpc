...
class Nominees
{
    ...
    public function UPDATE_NOMINEE($org, $pos, $name, $course, $year, $stud_id, $nom_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'>Update successful <a href='http://localhost/scpc/sandbox/add_nominees.php'><span class='glyphicon glyphicon-backward'></span> </a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_NOMINEE($nom_id) {
        ...
        if($stmt->execute()) {
            header("location: http://localhost/scpc/sandbox/add_nominees.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
