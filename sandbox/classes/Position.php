...
class Position
{
    ...
    public function UPDATE_POS($org, $pos, $pos_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'>Position was updated successfully.<a href='http://localhost/scpc/sandbox/add_pos.php'><span class='glyphicon glyphicon-backward'></span></a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_POS($pos_id) {
        ...
        if($stmt->execute()) {
            header("location: http://localhost/scpc/sandbox/add_pos.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
